...
/**
 * Live AR garment overlay rendered on top of the buyer's local camera PiP.
 *
 * Receives torso landmarks (normalized [0,1] image-space) from the backend
 * pose endpoint and composes an affine transform — rotate (shoulder line),
 * translate (torso center), scaleX (shoulder width), scaleY (shoulder→hip
 * length). Each new sample is animated into place over `tweenDurationMs`
 * so a 5–10 Hz pose poll feels like 60 Hz on screen.
 *
...
type Props = {
   ...
   * X here to match the rendered view. */
  mirror?: boolean;
...
export function ARGarmentOverlay(props: Props) {
  ...
  React.useEffect(() => {
    ...
    // Shoulder line: from image-left-shoulder → image-right-shoulder.
    const sdx = rsx - lsx;
    const sdy = rsy - lsy;
    const shoulderWidth = Math.hypot(sdx, sdy);

    // Body axis: shoulder midpoint → hip midpoint.
    ...
    if (shoulderWidth < 8 || bodyHeight < 10) {
      // Pose is degenerate (e.g. buyer is on top of camera or facing
      // away) — fade out instead of rendering a bizarre giant overlay.
      sv.show.value = withTiming(0, { duration: 200 });
      return;
    }

    // Rotate so the garment's local X axis aligns with the shoulder line.
    const rotationDeg = (Math.atan2(sdy, sdx) * 180) / Math.PI;

    // Scale: convert detected shoulder-width / body-height into multipliers
    // of our logical garment box. Apply the same expand factors the
    // backend warp uses so AR and snapshot agree visually.
    ...
    // Center of the garment box should sit at the torso midpoint, biased
    // slightly downward so the expanded hem covers the hips properly.
  ...
  const animatedStyle = useAnimatedStyle(() => ({
    ...
    transform: [
      ...
      // Rotate / scale around the garment-box center (RN transforms apply
      // right-to-left, and rotate/scale pivot on the view's origin, so we
      // pre-translate to put origin at the box's top-left and post-rotate
      // around its center by composing the translates above).
